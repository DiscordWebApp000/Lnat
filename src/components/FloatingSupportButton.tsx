...
export default function FloatingSupportButton() {
  ...
  const loadTickets = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      const userTickets = await supportService.getUserTickets(currentUser.uid);
  ...
  const openChat = async (ticket: SupportTicket) => {
    ...
    await supportService.markTicketAsRead(ticket.id, false);
  ...
  const createTicket = async () => {
    ...
    try {
      ...
      await supportService.sendMessage(messageData);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const updatedTickets = await supportService.getUserTickets(currentUser.uid);
  ...
  const getUnreadCount = () => {
    return tickets.filter(ticket => !ticket.isReadByUser).length;
  };
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  ...
                  disabled={isLoading || !newTicketSubject.trim() || !newTicketMessage.trim()}
